%%%-------------------------------------------------------------------
%% @doc env_api_key_plugin top level supervisor.
%% @end
%%%-------------------------------------------------------------------

-module(env_api_key_plugin_sup).
...
start_link() ->
  ...
  if
    ...
    true ->
      ...
      Keys1 = lists:delete({NewKey, "mgmt"}, Keys),
      vmq_config:set_global_env(vmq_server, ?ENV_API_KEYS, [{NewKey, "mgmt", undefined}|Keys1], true),
